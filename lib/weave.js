...
//---------------------------------------------------------------------------------------------------
require('at-js').defAnnotation('extention', {
    scope: 'var', build: function (_ctx, _argAry) {
        ...
        if (typeof def === 'object' && typeof(def = def[_ctx.refName]) === 'object') {
            if (_ctx.desc && _ctx.desc['returnEval']) {
                return "return require('" + MyPath + "').routeEval('" + def.mdef.name + "','" + def.def.name + "'" + _argAry + ");\n"
            } else {
                return "return require('" + MyPath + "').routeVal('" + def.mdef.name + "','" + def.def.name + "'" + _argAry + ");\n"
            }
        } else {
